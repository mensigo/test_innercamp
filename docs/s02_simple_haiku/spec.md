...
Если ввод представляет собой одну команду из списка `config.exit_commands`, то агент выводит "До свидания!" в stdout, пишет "AgentEnd" в лог (debug) и завершает выполнение.

Если ввод представляет собой одну команду из списка `config.help_commands`, то агент выводит справочное сообщение в stdout, пишет "AgentHelp" в лог (debug) и запрашивает ввод еще раз.

Если ввод представляет собой одну команду из списка `config.clear_commands`, то агент выводит "История сообщений очищена." в stdout, пишет "AgentClear" в лог (debug) и запрашивает ввод еще раз.
...
1. Агент пишет "Анализирую релевантность запроса.." в stdout и стейт "classify" в лог (debug).
...
Если ответ обработан успешно, и запрос пользователя **не** соответствует функционалу агента (код 101), то агент:
...
- проходит на Шаг 0 (запрос ввода от пользователя)

Если ответ обработан успешно, и запрос пользователя соответствует функционалу агента (код 100), то агент:
...
Если запрос падает с ошибкой (код 203), то агент:

- пишет "Ошибка при запросе LLM, завершаюсь.." в stdout
- пишет "select_tool_call // LLM Error: < error text >" в лог (critical)
- завершает выполнение

Если произошла ошибка при парсинге ответа LLM (код 202), то агент:

- пишет "Ошибка при разборе ответа LLM, завершаюсь.." в stdout
- пишет "select_tool_call // LLM Response Parse Error: < error text >" в лог (critical)
- завершает выполнение

Если ответ обработан успешно, и **не** удалось определить инструмент (код 201), то агент:
...
Если ответ обработан успешно, и удалось определить инструмент (код 200), то агент:
...
- пишет "Не совсем понял тему хайку. Какую использовать?" в stdout и добавляет в историю сообщений
...
- пишет "Инструмент generate_haiku проверен и готов к вызову. Тема: < theme >" в stdout
- пишет "validate_tool_call // Validation OK" в лог (debug)
- проходит на Шаг 4
...
- если ошибка health check, то пишет "Небольшие трудности при запросе на RAG сервис.. Инженеры уже работают над запуском сервиса.." в stdout и добавляет в историю сообщений
- если ошибка НЕ health check, то пишет "Произошла чудовищная ошибка при запросе на RAG сервис.. Тысяча извинений! Попробуем снова?" в stdout и добавляет в историю сообщений
...
- если ошибка health check, то пишет "Небольшие трудности при генерации хайку.. Инженеры уже работают над запуском сервиса.." в stdout и добавляет в историю сообщений
- если ошибка НЕ health check, то пишет "Произошла чудовищная ошибка при генерации хайку.. Тысяча извинений! Попробуем снова?" в stdout и добавляет в историю сообщений
- пишет доп. сообщения в зависимости от ситуации (подробнее ниже)
- проходит на Шаг 0 (запрос ввода от пользователя)
