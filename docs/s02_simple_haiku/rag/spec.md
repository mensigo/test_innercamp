...
Функция, которая используется в коде агента для запросов на RAG сервис.  
`answer_question(question: str, top_k: int = 2) -> dict`

При выполнении функции:
- если ошибка при запросе (health/search), то возвращает `{"error": "< error message >"}` (в т.ч. по таймауту)
- если запрос успешный, то возращает:
  ```yaml
  answer:
    type: string
    description: ответ по RAG
