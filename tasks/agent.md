...
Написать функцию agent, которая будет принимать на вход запрос пользователя и возвращать ответ агента (строка + экстра инфо).
...
Структура: // ДОПИСАТЬ ПАПКИ, УТОЧНИТЬ ФОРМАТЫ
...
Всего 30 студентов, каждый прошел 2 предмета и имеет оценку по каждому.  
Всего 3 предмета: Machine Learning, Probability Theory, Optimization Theory. // ПОДРОБНЕЕ ПРО ЭТИ ПРЕДМЕТЫ - ЕСТЬ ЖЕ ЕЩЕ
...
// ДОБАВИТЬ ТЗ НА ЭТИ СЛОВА ^^^

ВАЖНО:

- работа API rag_search завязана на индексе, для построения которого нужны эмбеддинги
...
- ответ агента, если в запросе на топ студентов нет основного предмета (напр., "топ студентов по философии")
...
// ADD GIGACHAT DOC / DESCRIPTION /CERTS - ЧЕЛОВЕК НИЧЕГО ЗНАЕТ, ПОДГОТОВИТЬ К ЖИЗНИ
